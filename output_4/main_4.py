...
def compute(iInput):
    ...
    margin = 100
    def currentLatency(cacheContent, e, videoId):
        l = -1
        for lc in e.latencyToCache:
            if videoId in cacheContent[int(lc[0])]:
                return int(lc[1])
        return e.latencyDatacenter
    ...
    for req in iInput.requestArray:
        for cache in iInput.endpointArray[req.endpointId].latencyToCache:
            ...
            if req.videoId in res.cacheContent[cacheId]:
                break

            if possibleToAddVideo(iInput, req.videoId, cacheId, remainingSize):
                curLatency = currentLatency(res.cacheContent, iInput.endpointArray[req.endpointId], req.videoId)
                optLatency = int(cache[1])
                if curLatency  >= optLatency + margin:
